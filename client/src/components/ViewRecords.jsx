...
export default function ViewRecords() {
    ...
    useEffect(() => {
        ...
        async function readData() {
            try {
                const doctorsArray = await contract.methods.getValidDoctors().call();
            ...
            <div className="overflow-x-auto">
                <table className="min-w-full divide-y divide-gray-200">
                    <thead className="bg-gray-50">
                        <tr>
                            <th scope="col" className="px-6 py-3 text-left text-xs font-platy text-gray-500 uppercase tracking-wider">Name</th>
                    ...
                    <tbody className="bg-white divide-y divide-gray-200">
                        {data.map((doctor, index) => (
                            <tr key={index}>
                                <td className="px-6 py-4 whitespace-nowrap text-black font-platy">{doctor.name}</td>
                                <td className="px-6 py-4 whitespace-nowrap text-black font-platy">{doctor.provider}</td>
                            </tr>
                        ))}
                    </tbody>
                </table>
